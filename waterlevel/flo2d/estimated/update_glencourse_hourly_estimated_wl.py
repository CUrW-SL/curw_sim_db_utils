...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(len(interpolated_ranwala_ts)):
            ranwala_wl = float(interpolated_ranwala_ts[i][1])
            glencourse_wl = 1.7983 * ranwala_wl - 1.6475
            glencourse_ts.append([interpolated_ranwala_ts[i][0], '%.3f' % glencourse_wl])

        if glencourse_ts is not None and len(glencourse_ts) > 0:
            TS.insert_data(timeseries=glencourse_ts, tms_id=tms_id, upsert=True)
            TS.update_latest_obs(id_=tms_id, obs_end=glencourse_ts[-1][1])
